...
describe('Zero Bank Demo Site Tests with Hooks', () => {
  // Before all tests
  before(() => {
    // Visit the Zero Bank demo site
  ...
  // After each test
  afterEach(() => {
    cy.log('Running teardown after each test');
  });

  // After all tests
  after(() => {

    cy.log('Running teardown after all tests');
  });

  it('Test Case 1: Verify Homepage Title', () => {
    // Verify the title of the homepage
    cy.title().should('eq', 'Zero - Personal Banking - Loans - Credit Card');
    cy.screenshot('assertion fail');
  });

  it('Test Case 2: Verify Login Link', () => {
    // Verify the presence of the login link
    cy.get('#signin_button').should('be.visible');
    //cy.get('#signin_button').should('be.visible').and('have.text', 'Signin ');
...
//Page reloads
// describe('Page Reloads on Zero Bank Demo Site', () => {
//   beforeEach(() => {
//     // Visit the Zero Bank demo site before each test
//     cy.visit('http://zero.webappsecurity.com/');
//   });

//   // it('Should perform a standard page reload', () => {
//   //   // Reload the page
//   //   cy.reload();
//   //   // Assert that the page has been reloaded
//   //   cy.url().should('eq', 'http://zero.webappsecurity.com/');
//   // });

//   // it('Should perform a hard page reload', () => {
//   //   // Reload the page without using cache (hard reload)
//   //   cy.reload(true);
//   //   // Assert that the page has been reloaded
//   //   cy.url().should('eq', 'http://zero.webappsecurity.com/');
//   // });

//   // it('Should perform a soft page reload', () => {
//   //   // Soft reload by navigating to the same page
//   //   cy.visit('http://zero.webappsecurity.com/');
//   //   // Assert that the page has been reloaded
//   //   cy.url().should('eq', 'http://zero.webappsecurity.com/');
//   // });

//   // it('Should perform a soft page reload using back and forward navigation', () => {
//   //   // Perform a soft reload by navigating back and then forward
//   //   cy.go('back');
//   //   cy.go('forward');
//   //   // Assert that the page has been reloaded
//   //   cy.url().should('eq', 'http://zero.webappsecurity.com/');
...
//   // it('Should reload the page by clicking a link', () => {
//   //   // Click on a link to reload the page
//   //   cy.contains('Zero Bank').click();
//   //   // Assert that the page has been reloaded
//   //   cy.url().should('eq', 'http://zero.webappsecurity.com/index.htm');
//   //   cy.screenshot('Failed Assertion Website Reload');
//   // });
// });
